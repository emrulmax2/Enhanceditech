...
class Gender_management extends CI_Controller {
	...
	public function index(){
		...
		if($this->session->userdata('label')=="staff"){
		  		...
		  		if(!empty($staff_access['staff_privileges']['gender_management']) && count($staff_access['staff_privileges']['gender_management'])>0) $priv = $data['priv'] = $staff_access['staff_privileges']['gender_management'];				
        ...
        if($this->input->post('ref')=="edit" && (!empty($priv[2]) || $this->session->userdata('label')=="admin")){
			...
			$insertedid=$this->student_gender->update($args);
	       $data['error']=0; 
	       if($insertedid){
	       		
	       		$data['message'] = "<div class='alert alert-success'> Gender has been successfully updated. </div>";	   
        ...
        //}else if($this->input->post('ref')=="add"  && (!empty($staff_privileges_course_management['course_mng_add']) || $this->session->userdata('label')=="admin")){
			...
			//var_dump($args);
			$insertedid=$this->student_gender->add($args);
	       $data['error']=0; 
	       if($insertedid){
	       		
	       		$data['message'] = "<div class='alert alert-success'> Gender has been successfully added. </div>";	   
	    ...
	    if(!empty($varsessioncheck_id) && ($action==NULL || $action=="list")){
	        ...
	        $data['bodytitle']       =   "Gender Management";
	        $data['faicon']          =   "fa-female";
	        $data['breadcrumbtitle'] =   "Dashboard > Gender Management";             
	        ...
	        $this->load->view('staff/gender_body_list');
	        ...
	        $this->load->view('staff/dashboard_topmenu');
	        $this->load->view('staff/dashboard_sidebar');
	        $this->load->view('staff/gender_body_form');
	        $this->load->view('staff/other_footer');
	        ...
	        $data['gender'] = $this->student_gender->get_by_ID($id);
	        
	        $data['ref'] = 'edit';
	        $data['ref_id'] = $data['gender']['id'];
