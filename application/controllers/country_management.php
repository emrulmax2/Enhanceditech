...
class Country_management extends CI_Controller {
	...
	public function index(){
        ...
        if($this->session->userdata('label')=="staff"){
                  ...
                  if(!empty($staff_access['staff_privileges']['country_management']) && count($staff_access['staff_privileges']['country_management'])>0) $priv = $data['priv'] = $staff_access['staff_privileges']['country_management'];
        ...
        if($this->input->post('ref')=="edit" && (!empty($priv[2]) || $this->session->userdata('label')=="admin")){
			...
			foreach($this->input->post() as $k=>$v){
				...
				if($k=="country_name" || $k=="iso_code_2" || $k=="iso_code_3"){ $$k = tinymce_encode($v); $args[$k] = $$k; }
			}
			$insertedid=$this->country->update($args);
	       $data['error']=0; 
	       if($insertedid){
	       		
	       		$data['message'] = "<div class='alert alert-success'> Country has been successfully updated. </div>";	   
        ...
        //}else if($this->input->post('ref')=="add"  && (!empty($staff_privileges_course_management['course_mng_add']) || $this->session->userdata('label')=="admin")){
			...
			//var_dump($args);
			$insertedid=$this->country->add($args);
	       $data['error']=0; 
	       if($insertedid){
	       		
	       		$data['message'] = "<div class='alert alert-success'> Country has been successfully added. </div>";	   
	    ...
	    if(!empty($varsessioncheck_id) && ($action==NULL || $action=="list")){
	        ...
	        $data['bodytitle']       =   "Country Management";
	        $data['faicon']          =   "fa-globe";
	        $data['breadcrumbtitle'] =   "Dashboard > Country Management";             
	        ...
	        $this->load->view('staff/country_list');
	        ...
	        $this->load->view('staff/dashboard_topmenu');
	        $this->load->view('staff/dashboard_sidebar');
	        $this->load->view('staff/country_form');
	        $this->load->view('staff/other_footer');
	        ...
	        $data['country'] = $this->country->get_by_ID($id);
	        
	        $data['ref'] = 'edit';
	        $data['ref_id'] = $data['country']['id'];
