...
class Inbox_management extends CI_Controller {   
...
public function index(){
    ...
    $data['bodytitle']	    =	"Applicant Inbox";
    $data['breadcrumbtitle']=	"Dashboard > Inbox";
         ...
         if(isset($inbox_mark) && $inbox_mark!="") {
            ...
            if(isset($affectedrow) && $affectedrow ==TRUE) {
             ...
             foreach($student_data_ids as $email => $ids) {
                ...
                foreach($ids as $id):
                    $inbox = $this->lcc_inbox->get_by_student_ID($id);           
                endforeach;
                foreach($inbox as $lists) {
                  foreach ($lists as $inbox_row) {
    ...
    foreach($student_data_ids as $email => $ids) {
        ...
        $this->load->view('student/inbox/inbox_view');
        $this->load->view('other_footer'); 

          
        
    }else if(!empty($varsessioncheck_id) && $label == "student" && $action=="trash"){
    ...
    foreach($student_data_ids as $email => $ids) {
        foreach($ids as $id) {
            $data["inbox"] = $this->lcc_inbox->get_trash_by_student_ID($id);
        }
    }           
        ...
        $this->load->view('student/dashboard_topmenu');
        $this->load->view('student/dashboard_sidebar');
        $this->load->view('student/inbox/trash_view');
        $this->load->view('other_footer'); 
    ...
    foreach($student_data_ids as $email => $ids) {
        ...
        $this->load->view('student/dashboard_topmenu');
        $this->load->view('student/dashboard_sidebar');
        $this->load->view('student/inbox/inbox_details');
        $this->load->view('other_footer'); 
