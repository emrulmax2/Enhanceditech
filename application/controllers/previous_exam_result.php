...
class Previous_exam_result extends CI_Controller {
	...
	public function index(){
        ...
        if($this->session->userdata('label')=="staff"){
                  ...
                  if(!empty($staff_access['staff_privileges']['exam_result_management']) && count($staff_access['staff_privileges']['exam_result_management'])>0) $priv = $data['priv'] = $staff_access['staff_privileges']['exam_result_management'];                
        ...
        if($action=="search"){
        	    ...
        	    if($_POST){			
					...
					foreach($this->input->post() as $k=>$v){
						...
						if($k!="ref" && $k!="ref_id" && $k!="course_relation_id"){  if(!empty($v)){ $$k=tinymce_encode($v); $terms[$k] = $$k; }  }
					}

					$sesData['prev_exam_result_student_search']['terms']=array();
					$this->session->set_userdata($sesData);
					$sesData = array();
				    $sesData['prev_exam_result_student_search']['terms'] = $terms;
				    $this->session->set_userdata($sesData);					
				
        		}	
        	
        	   $sesData   = $this->session->userdata("prev_exam_result_student_search");
               $terms     = $sesData['terms'];
...
*/
	    ...
	    if(!empty($varsessioncheck_id) && $action!="search"){
	        ...
	        $data['bodytitle']       =   "Previous Exam Result";
	        $data['faicon']          =   "fa-trophy";
	        $data['breadcrumbtitle'] =   "Dashboard > Previous Exam Result";             
	        ...
	        $this->load->view('staff/dashboard_topmenu');
	        $this->load->view('staff/dashboard_sidebar');
	        $this->load->view('staff/prev_exam_result_body_form.php');
	        $this->load->view('staff/other_footer');
	        
		}else if(!empty($varsessioncheck_id) && $action=="search"){
	            
	        $data['bodytitle']       =   "Previous Exam Result Management";
	        $data['faicon']          =   "fa-trophy";
	        $data['breadcrumbtitle'] =   "Dashboard > Previous Exam Result Management";             	        
            ...
            $data['prev_exam_result'] = $this->exam_result_prev->get_search_result($terms);
            
            //var_dump($data['prev_exam_result']);		
