...
class Job_type_management extends CI_Controller {
	...
	public function index(){
		...
		if($this->session->userdata('label')=="staff"){
		  		...
		  		if(!empty($staff_access['staff_privileges']['job_type_management']) && count($staff_access['staff_privileges']['job_type_management'])>0) $priv = $data['priv'] = $staff_access['staff_privileges']['job_type_management'];				
        ...
        if($this->input->post('ref')=="edit" && (!empty($priv[2]) || $this->session->userdata('label')=="admin")){
			...
			$insertedid=$this->job_type->update($args);
	       $data['error']=0; 
	       if($insertedid){
	       		
	       		$data['message'] = "<div class='alert alert-success'> Title has been successfully updated. </div>";	   
        ...
        //}else if($this->input->post('ref')=="add"  && (!empty($staff_privileges_course_management['course_mng_add']) || $this->session->userdata('label')=="admin")){
			...
			//var_dump($args);
			$insertedid=$this->job_type->add($args);
	       $data['error']=0; 
	       if($insertedid){
	       		
	       		$data['message'] = "<div class='alert alert-success'> Job type has been successfully added. </div>";	   
	    ...
	    if(!empty($varsessioncheck_id) && ($action==NULL || $action=="list")){
	        ...
	        $data['bodytitle']       =   "Job Type Management";
	        $data['faicon']          =   "fa-info";
	        $data['breadcrumbtitle'] =   "Dashboard > Job Type Management";             

	        $data['job_type'] = $this->job_type->get_all();
	        ...
	        $this->load->view('staff/job_induction/job_type_body_list');
	        ...
	        $this->load->view('staff/dashboard_topmenu');
	        $this->load->view('staff/dashboard_sidebar');
	        $this->load->view('staff/job_induction/job_type_body_form');
	        $this->load->view('staff/other_footer');
	        ...
	        $data['job_type'] = $this->job_type->get_by_ID($id);
	        
	        $data['ref'] = 'edit';
	        $data['ref_id'] = $data['job_type']['id'];
