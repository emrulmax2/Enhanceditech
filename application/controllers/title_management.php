...
class Title_management extends CI_Controller {
	...
	public function index(){
        ...
        if($this->session->userdata('label')=="staff"){
                  ...
                  if(!empty($staff_access['staff_privileges']['title_management']) && count($staff_access['staff_privileges']['title_management'])>0) $priv = $data['priv'] = $staff_access['staff_privileges']['title_management'];                
        ...
        if($this->input->post('ref')=="edit" && (!empty($priv[2]) || $this->session->userdata('label')=="admin")){
			...
			$insertedid=$this->student_title->update($args);
	       $data['error']=0; 
	       if($insertedid){
        ...
        //}else if($this->input->post('ref')=="add"  && (!empty($staff_privileges_course_management['course_mng_add']) || $this->session->userdata('label')=="admin")){
			...
			//var_dump($args);
			$insertedid=$this->student_title->add($args);
	       $data['error']=0; 
	       if($insertedid){
	       		
	       		$data['message'] = "<div class='alert alert-success'> Title has been successfully added. </div>";	   
	    ...
	    if(!empty($varsessioncheck_id) && ($action==NULL || $action=="list")){
	        ...
	        $data['bodytitle']       =   "Title Management";
	        $data['faicon']          =   "fa-info";
	        $data['breadcrumbtitle'] =   "Dashboard > Title Management";             
	        ...
	        $this->load->view('staff/title_body_list');
	        ...
	        $this->load->view('staff/dashboard_topmenu');
	        $this->load->view('staff/dashboard_sidebar');
	        $this->load->view('staff/title_body_form');
	        $this->load->view('staff/other_footer');
	        ...
	        $data['title'] = $this->student_title->get_by_ID($id);
