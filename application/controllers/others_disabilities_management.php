...
class Others_disabilities_management extends CI_Controller {
	...
	public function index(){
        ...
        if($this->session->userdata('label')=="staff"){
                  $staff_access = $this->login->getStaffAccess($this->session->userdata('uid'));                  
                   if(!empty($staff_access['staff_privileges']['others_disabilities_management']) && count($staff_access['staff_privileges']['others_disabilities_management'])>0) $priv = $data['priv'] = $staff_access['staff_privileges']['others_disabilities_management'];                
        ...
        if($this->input->post('ref')=="edit" && (!empty($priv[2]) || $this->session->userdata('label')=="admin")){
			...
			foreach($this->input->post() as $k=>$v){
				...
				if($k=="name" || $k=="code" || $k=="sign_url"){ $$k = tinymce_encode($v); $args[$k] = $$k; }
			}
			$insertedid=$this->student_others_disabilities->update($args);
	       $data['error']=0; 
	       if($insertedid){
	       		
	       		$data['message'] = "<div class='alert alert-success'> Disabilities has been successfully updated. </div>";	   
        ...
        //}else if($this->input->post('ref')=="add"  && (!empty($staff_privileges_course_management['course_mng_add']) || $this->session->userdata('label')=="admin")){
			...
			//var_dump($args);
			$insertedid=$this->student_others_disabilities->add($args);
	       $data['error']=0; 
	       if($insertedid){
	       		
	       		$data['message'] = "<div class='alert alert-success'> Disabilities has been successfully added. </div>";	   
	    ...
	    if(!empty($varsessioncheck_id) && ($action==NULL || $action=="list")){
	        ...
	        $data['bodytitle']       =   "Disabilities Management";
	        $data['faicon']          =   "fa-book";
	        $data['breadcrumbtitle'] =   "Dashboard > Disabilities Management";

	        $data['title_list'] = $this->student_others_disabilities->get_all();
	        ...
	        $this->load->view('staff/disabilities_list');
	        ...
	        $this->load->view('staff/dashboard_topmenu');
	        $this->load->view('staff/dashboard_sidebar');
	        $this->load->view('staff/disabilities_form');
	        $this->load->view('staff/other_footer');
	        ...
	        $data['title'] = $this->student_others_disabilities->get_by_ID($id);
	        
	        $data['ref'] = 'edit';
	        $data['ref_id'] = $data['title']['id'];
