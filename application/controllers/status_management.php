...
class Status_management extends CI_Controller {
...
//////////////////////////////////////////////////////	    
		...
		if($this->session->userdata('label')=="staff"){
		  		...
		  		if(!empty($staff_access['staff_privileges']['status_management']) && count($staff_access['staff_privileges']['status_management'])>0) $priv = $data['priv'] = $staff_access['staff_privileges']['status_management'];				
        ...
        if($this->input->post('ref')=="edit" && (!empty($priv[2]) || $this->session->userdata('label')=="admin")){
			...
			$insertedid=$this->status->update($args);
	       $data['error']=0; 
	       if($insertedid){
	       		
	       		$data['message'] = "<div class='alert alert-success'> Status has been successfully updated. </div>";	   
        ...
        //}else if($this->input->post('ref')=="add"  && (!empty($staff_privileges_course_management['course_mng_add']) || $this->session->userdata('label')=="admin")){
			...
			//var_dump($args);
			$insertedid=$this->status->add($args);
	       $data['error']=0; 
	       if($insertedid){
	       		
	       		$data['message'] = "<div class='alert alert-success'> Status has been successfully added. </div>";	   
	    ...
	    if(!empty($varsessioncheck_id) && ($action==NULL || $action=="list")){
	        ...
	        $data['bodytitle']       =   "Status Management";
	        $data['faicon']          =   "fa-info";
	        $data['breadcrumbtitle'] =   "Dashboard > Status Management";             

	        $data['title_list'] = $this->status->get_all();
	        ...
	        $this->load->view('staff/status_body_list');
	        ...
	        $this->load->view('staff/dashboard_topmenu');
	        $this->load->view('staff/dashboard_sidebar');
	        $this->load->view('staff/status_body_form');
	        $this->load->view('staff/other_footer');
	        ...
	        $data['title'] = $this->status->get_by_ID($id);
