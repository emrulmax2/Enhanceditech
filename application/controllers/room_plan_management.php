...
class Room_plan_management extends CI_Controller {
	...
	public function index(){
        ...
        if($this->session->userdata('label')=="staff"){
                  ...
                  if(!empty($staff_access['staff_privileges']['room_plan_management']) && count($staff_access['staff_privileges']['room_plan_management'])>0) $priv = $data['priv'] = $staff_access['staff_privileges']['room_plan_management'];                
        ...
        if($this->input->post('ref')=="edit" && (!empty($priv[2]) || $this->session->userdata('label')=="admin")){
			...
			$insertedid=$this->room_plan->update($args);
	       $data['error']=0; 
	       if($insertedid){
	       		
	       		$data['message'] = "<div class='alert alert-success'> Room plan has been successfully updated. </div>";	   
        ...
        //}else if($this->input->post('ref')=="add"  && (!empty($staff_privileges_course_management['course_mng_add']) || $this->session->userdata('label')=="admin")){
			...
			//var_dump($args);
			$insertedid=$this->room_plan->add($args);
	       $data['error']=0; 
	       if($insertedid){
	       		
	       		$data['message'] = "<div class='alert alert-success'> Room plan has been successfully added. </div>";	   
	    ...
	    if(!empty($varsessioncheck_id) && ($action==NULL || $action=="list")){
	        ...
	        $data['bodytitle']       =   "Room Plan Management";
	        $data['faicon']          =   "fa-building-o";
	        $data['breadcrumbtitle'] =   "Dashboard > Room Plan Management";             
	        ...
	        $this->load->view('staff/room_plan_body_list');
	        ...
	        $this->load->view('staff/dashboard_topmenu');
	        $this->load->view('staff/dashboard_sidebar');
	        $this->load->view('staff/room_plan_body_form');
	        $this->load->view('staff/other_footer');
	        ...
	        $data['room_plan'] = $this->room_plan->get_by_ID($id);
	        
	        $data['ref'] = 'edit';
	        $data['ref_id'] = $data['room_plan']['id'];
