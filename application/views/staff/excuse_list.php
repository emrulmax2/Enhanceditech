...
$(document).ready(function(){
	...
	$('#checkbox99999999999, tbody.class-plan-list-body td .class-plan-id').click(function(){
		...
		var i =0;
		var id = [];
		$.each($('.gradeA').find('.class-plan-id'),function(){
		...
		//alert(id);
		if(i>0){
			//$('.btn-area').find('.vew-feed-attendance').remove(); 
			$('.btn-area').find('.vew-feed-attendance').html("<?php if((!empty($priv[2]) || $this->session->userdata('label')=="admin")){ ?><a class='btn btn-md btn-primary btn-assign-student' href='?action=add_to_archive&id="+id+"'><i class=\"fa fa-plus\"></i> Add to archive</a><?php } ?>");
			
		}else{
			$('.btn-area').find('.vew-feed-attendance').html("");
		}
...
// }
                ...
                <div class="row">
	                ...
	                <?php if((!empty($priv[1]) || $this->session->userdata('label')=="admin")){ ?><a href="?action=archived" class="btn btn-success"><i class="fa fa-eye"></i> View Archived</a> <?php } ?>
	                </div>
...
?>                	
               ...
               <div class="col-lg-12">
	                ...
	                <table class="dTable display">
	                    <thead>
	                        <tr>
	                            <th><div class='checkbox checkbox-primary'><input id='checkbox99999999999' type='checkbox' class='form-control'><label for='checkbox99999999999'>Student ID</label></div></th>
	                            <th>Name</th>
	                            <th>Dates</th>
	                            <th>Reason</th>
	                            <th>Docs</th>
	                    ...
	                    <tbody class="class-plan-list-body">
	                        <?php
                                ...
                                $i=1;
	                            foreach($attendance_excuse_list as $k => $v){
	             
	                                echo "<tr  class='gradeA'>";                 
	                                echo "<td><div class='checkbox checkbox-primary'><input name='class_id[]' id='checkbox".$v["id"]."' type='checkbox' class='form-control class-plan-id' value='".$v["id"]."'><label for='checkbox".$v["id"]."'>".$this->register->get_registration_no_by_ID($v['register_id'])."</label></div></div></td>";
