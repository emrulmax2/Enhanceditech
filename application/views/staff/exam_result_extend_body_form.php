...
<?php			
    ...
    $('.add-all-result-data-btn').click(function(){
		...
		$('.update-result-loading').show();
		var num_row=0; var num_success=0;
		$.each($('tbody.reg-std-row').find('tr'),function(){
			num_row++;
		});
		...
		$.each($('tbody.reg-std-row').find('tr'),function(){
			...
			//alert($(this).attr('group_name'));			
			var group_name 		= $(this).attr('group_name');
			var register_id 	= $(this).attr('register_id');
			var examresult_id 	= $(this).attr('examresult_id');
			var percentage 		= $(this).find('input.percentage').val();
			var grade 			= $(this).find('input.grade').val();
			var marks 			= $(this).find('input.marks').val();
			var PaperID 		= $(this).find('input.PaperID').val();

				url = getURL()+'/index.php/ajaxall/';
				$.ajax({
				   type: "POST",
				   data: {action: "examResultPostAllResult", semister_id:semister_id, course_id:course_id, coursemodule_id:coursemodule_id, percentage:percentage, grade:grade, marks:marks, PaperID:PaperID, examresult_id:examresult_id, register_id:register_id, group_name:group_name  },
				   url: url,
				   success: function(msg){
				     ///$('.message').html(msg);
				     //alert(msg);
				     ...
				     if(num_success==num_row){
						$('.update-result-loading').hide();
						$('.message').html("<div class=\"alert alert-success\"><p><span class=\"fa fa-check\"></span> Exam result successfully updated.</p></div>");
						window.location = getURL()+'/index.php/exam_result_management_extend/?action=search'; 
				     }
                ...
                <div class="row">
		                ...
		                <div class="col-lg-12">
		                        ...
		                        <div class="row">
		                        	...
		                        	<div class="col-xs-12">
		                        		<form role="form" method="post" action="<?php echo base_url(); ?>index.php/exam_result_management_extend/?action=search">
			                					...
			                					<div class="row clearfix">
			                						...
			                						<div class="col-sm-12 ">
														...
														<div class="text-right btn-area col-sm-6 no-pad-right">
															<img src="<?php echo base_url(); ?>images/loading.gif" class="loading update-result-loading">
														    <?php if(!empty($priv[0]) || $this->session->userdata('label')=="admin"){ ?><button type="button" class="btn btn-default btn-danger add-all-result-data-btn"><i class="fa fa-plus-circle"></i> Update Result</button><?php } ?>
				                							<?php if(!empty($priv[1]) || $this->session->userdata('label')=="admin"){ ?><button type="button" class="btn btn-default btn-success publish-date-btn"><i class="fa fa-calendar"></i> Publish Date</button><?php } ?>
				                							<?php if(!empty($priv[2]) || $this->session->userdata('label')=="admin"){ ?><button type="button" class="btn btn-default btn-primary print-btn"><i class="fa fa-print"></i> Print</button><?php } ?>
														</div>
                                                        <div class="clearfix"></div>
			                						</div>
			                						<div class="clearfix"></div>
...
<?php
													...
													foreach($reg_student_list as $register_id){
														...
														if(!empty($std_data['grade'])) echo"<td><input type='text' class='grade form-control' name='grade' value='".$std_data['grade']."'></td>"; else echo"<td><input type='text' class='grade form-control' name='grade' value=''></td>"; 		
														if(!empty($std_data['percentage'])) echo"<td><input type='text' class='percentage form-control' name='percentage' value='".$std_data['percentage']."'></td>"; else echo"<td><input type='text' class='percentage form-control' name='percentage' value=''></td>"; 		
														if(!empty($std_data['PaperID'])) echo"<td><input type='text' class='PaperID form-control' name='PaperID' value='".$std_data['PaperID']."'></td>"; else echo"<td><input type='text' class='PaperID form-control' name='PaperID' value=''></td>"; 		
														if(!empty($std_data['marks'])) echo"<td><input type='text' class='marks form-control' name='marks' value='".$std_data['marks']."'></td>"; else echo"<td><input type='text' class='marks form-control' name='marks' value=''></td>"; 		
                ...
                <div class="modal fade" id="publishdateModal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
                  <div class="modal-dialog">
                    <div class="modal-content">
                      ...
                      <button type="button" class="btn btn-primary submit-publish-date-btn">Submit</button>
