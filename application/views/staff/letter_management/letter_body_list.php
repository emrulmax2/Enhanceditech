...
function recallRemove(){
    $('.remove-btn').click(function(){
        ...
        $('#myModal').find('.confirm-btn').attr('onclick','RemoveFromList(\''+id+'\',\'letter_set\')');
        $('#myModal').css({'top':'30%'});
        $('#myModal').modal('hide');
...
?>               	
               ...
               <div class="col-lg-12">
	                ...
	                <h4><i class="fa fa-list"></i> Letter List </h4>
	                <table class="dTable display">
	                    ...
	                    <tbody>
	                        <?php
	                            ...
	                            foreach($letter_list as $k => $letter){
	             
	                                echo "<tr  class='gradeA'>";                 
	                                echo "<td>".$letter["letter_title"]."</td>";
	                                //if(empty($staff_privileges_course_management['course_mng_edit']) && $this->session->userdata('label')!="admin")
	                                //echo"<td></td>";
	                                //else
	                                echo "<td>";
                                    
                                    if( !empty($priv[2]) || $this->session->userdata('label')=="admin" ) echo "<a href='".base_url()."index.php/registration/letter_management/?action=edit&id=".$letter["id"]."' class='btn btn-sm btn-success margin-right-5'><i class='fa fa-pencil-square-o'></i></a>";
                                            
                                    if( !empty($priv[3]) || $this->session->userdata('label')=="admin" ) echo"<a href='javascript:void(0);' class='btn btn-sm btn-danger remove-btn' id='".$letter["id"]."'><i class='fa fa-times'></i></a>";
