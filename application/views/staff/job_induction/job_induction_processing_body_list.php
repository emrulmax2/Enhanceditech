...
function recallRemove(){
    $('.remove-btn').click(function(){
        ...
        $('#myModal').find('.confirm-btn').attr('onclick','RemoveFromList(\''+id+'\',\'job_induction_process\')');
        $('#myModal').css({'top':'30%'});
        $('#myModal').modal('hide');
...
?>                
	                ...
	                <h4><i class="fa fa-list"></i> Job Induction Process List </h4>
	                <table class="dTable display">
	                    ...
	                    <tbody>
	                        <?php
	                            ...
	                            foreach($job_induction_processing_list as $k => $v){
	             
	             					$induction = $this->job_induction->get_by_ID($v['job_induction_id']);
	             					
	                                echo "<tr  class='gradeA'>";                 
	                                echo "<td>".ucfirst($induction["name"])."</td>";
	                                echo "<td>".date("d-m-Y",strtotime($induction["date"]))."</td>";
	                                if(!empty($v["jobs_list"])){
	                                	$jobs_list = unserialize($v["jobs_list"]);
	                                	echo "<td>";
	                                	foreach($jobs_list as $job){
											echo ucfirst($this->jobs->get_name_by_id($job)).", ";	
	                                	}
	                                	echo"</td>";
									}else
	                                	echo "<td>None</td>";	
	                                
	                                echo "<td>".ucwords($v["students_notified"])."</td>";	
	                                echo "<td>";
	                                if(!empty($priv[2]) || $this->session->userdata('label')=="admin")
	                                echo "<a href='".base_url()."index.php/job_induction/job_induction_processing_management/?action=edit&id=".$v["id"]."' class='btn btn-sm btn-success margin-right-5'><i class='fa fa-pencil-square-o'></i></a>";
