...
function recallRemove(){
    $('.remove-btn').click(function(){
        ...
        $('#myModal').find('.confirm-btn').attr('onclick','RemoveFromList(\''+id+'\',\'agent\')');
        $('#myModal').css({'top':'30%'});
        $('#myModal').modal('hide');
	                    ...
	                    <tbody>
	                        <?php
	                            ...
	                            foreach($agent_list as $k => $agent){
	                                ...
	                                echo "<td>".$agent["agent_nick_name"]."</td>";
	                                echo "<td>".$agent["email_address"]."</td>";
	                                echo "<td>".$agent["agent_status"]."</td>";
	                                if(empty($staff_privileges_agent_management['agent_mng_edit']) && $this->session->userdata('label')!="admin")
	                                echo"<td></td>";
	                                else	                                
	                                echo "<td>";
	                            if(!empty($priv[2]) || $this->session->userdata('label')=="admin")
	                            echo "<a href='".base_url()."/index.php/agent_management/?action=edit&id=".$agent["id"]."' class='btn btn-sm btn-success margin-right-5'><i class='fa fa-pencil-square-o'></i></a>";
	                        	
	                        	if(!empty($priv[3]) || $this->session->userdata('label')=="admin")
	                            echo "<a href='javascript:void(0);' class='btn btn-sm btn-danger remove-btn' id='".$agent["id"]."'><i class='fa fa-times'></i></a></td>";
	                                echo "</tr>";
