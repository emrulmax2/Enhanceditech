...
function recallRemove(){
    $('.remove-btn').click(function(){
        ...
        $('#myModal').find('.confirm-btn').attr('onclick','RemoveFromList(\''+id+'\',\'semister\')');
        $('#myModal').css({'top':'30%'});
        $('#myModal').modal('hide');
...
?> 	                
                		<a class="btn btn-md btn-primary"  href="<?php echo base_url(); ?>/index.php/semester_management/?action=add"><i class="fa fa-plus"></i> Add semester</a>
<?php
}                			
?>                		
...
?>                
	                <h4><i class="fa fa-list"></i> Semester List </h4>
	                <table class="dTable display">
	                    ...
	                    <tbody>
	                        <?php
	                            ...
	                            foreach($semester_list as $k => $semester){
	             
	                                echo "<tr  class='gradeA'>";                 
	                                echo "<td>".$semester["semister_name"]."</td>";
	                                
	                                echo "<td>";
	                                if(!empty($priv[2]) || $this->session->userdata('label')=="admin")	
	                                echo "<a href='".base_url()."/index.php/semester_management/?action=edit&id=".$semester["id"]."' class='btn btn-sm btn-success margin-right-5'><i class='fa fa-pencil-square-o'></i></a>";
	                                if(!empty($priv[3]) || $this->session->userdata('label')=="admin")
	                                echo "<a href='javascript:void(0);' class='btn btn-sm btn-danger remove-btn' id='".$semester["id"]."'><i class='fa fa-times'></i></a>";
