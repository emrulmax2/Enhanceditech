...
?>               		
                		<a class="btn btn-md btn-primary"  href="<?php echo base_url(); ?>index.php/course_module_relation_management/?action=add"><i class="fa fa-plus"></i> Add New Module Relation</a>
<?php
}                			
?>                		
...
?>                
	                <h4><i class="fa fa-list"></i> Course Level List </h4>
	                <table class="dTable display">
	                    ...
	                    <tbody>
	                        <?php
	                            ...
	                            foreach($c_level_list as $k => $v){
	                            	$level = $this->course_level->get_by_course_ID($v["id"]);
	             
	                                echo "<tr  class='gradeA'>";                 
	                                echo "<td>".$v["course_name"]."</td>";
	                                ?>
	                                <td width="450px;">
	                                	<?php
	                                	if(!empty($level)) {
		                                	$x = 1; 
		                                	foreach ($level as $y => $z) {
		                                		$l[] = $x.". ".$z['name'];
		                                	$x++;
		                                	}
	                                ...
	                                if(!empty($priv[2]) || $this->session->userdata('label')=="admin"){
		                                ...
		                                echo "href='".base_url()."index.php/course_module_relation_management/?action=edit&course_id=".$v["id"]."&l=1' class='btn btn-sm btn-success margin-right-5'><i class='fa fa-pencil-square-o'></i>
	                                		</a>";
									}
