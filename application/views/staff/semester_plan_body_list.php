...
function recallRemove(){
    $('.remove-btn').click(function(){
        ...
        $('#myModal').find('.confirm-btn').attr('onclick','RemoveFromList(\''+id+'\',\'semester_plan\')');
        $('#myModal').css({'top':'30%'});
        $('#myModal').modal('hide');
...
?>               	
               ...
               <div class="col-lg-12">
	                ...
	                <h4><i class="fa fa-list"></i> Semester Plan List </h4>
	                    ...
	                    <tbody>
	                        <?php
	                            ...
	                            foreach($semester_plan_list as $k => $v){
	                                ...
	                                echo "<td>".tohrdate($v["start_date"])."</td>";
	                                echo "<td>".tohrdate($v["end_date"])."</td>";
	                                echo "<td>".tohrdate($v["teaching_start"])."</td>";
	                                echo "<td>".tohrdate($v["teaching_end"])."</td>";
	                                echo "<td>".tohrdate($v["revision_start"])."</td>";
	                                echo "<td>".tohrdate($v["revision_end"])."</td>";
	                                //if(empty($staff_privileges_course_management['course_mng_edit']) && $this->session->userdata('label')!="admin")
	                                //echo"<td></td>";
	                                //else
	                                echo "<td>";
                                    if( (!empty($priv[2]) || $this->session->userdata('label')=="admin") ) echo"<a href='".base_url()."index.php/semester_plan_management/?action=edit&id=".$v["id"]."' class='btn btn-sm btn-success margin-right-5'><i class='fa fa-pencil-square-o'></i></a>";
