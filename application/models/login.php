  ...
  class Login extends CI_Model {
    ...
    */
    function checkUserType($username){
        ...
        if($status=="active") {
            $qcheck_register="";
            $qcheck_register=$this->db->query("SELECT * FROM `{$this->fixidb->register}` WHERE `student_data_id`=\"$stu_id\" LIMIT 1");     // for student
            if (!empty($qcheck_register) && $qcheck_register->num_rows() > 0)
            { $user_type ="registered"; } else $user_type ="student";
    ...
    */
    ...
    function get_user_by_activate_session_id($activate_session_id="")
    {
       ...
       $qcheck=$this->db->query("SELECT * FROM `{$this->fixidb->student_data}` WHERE `activate_session_id`=\"$activate_session_id\" AND `student_status` = 'active'  ORDER BY `id` ASC LIMIT 1");     // for student
       ...
       $user = array();
        ...
        if (!empty($qcheck) && $qcheck->num_rows() > 0)
        {
                ...
                $user['uid'] = $qcheck->row()->id;
                $user['username'] = $qcheck->row()->student_email;
    ...
    * 
    */
    function get_registered_user($username="",$password="")
    {
        ...
        $qcheck_register="";
       /* check the user is a student or not*/
       $qcheck_register=$this->db->query("SELECT * FROM `{$this->fixidb->register}` WHERE `registration_no`=\"$username\" LIMIT 1");     // for student
       if (!empty($qcheck_register) && $qcheck_register->num_rows() > 0)
       {
            $student_data_id = $qcheck_register->row()->student_data_id;
           // $ID=$qcheck->row()->id;
            $qcheck=$this->db->query("SELECT * FROM `{$this->fixidb->student_data}` WHERE `id`=\"$student_data_id\" AND `student_status` = 'active'  ORDER BY `id` ASC LIMIT 1");     // for student
       }     
...
function getStaffAccess($ID){
	...
	$qcheck=$this->db->query("SELECT * FROM `{$this->fixidb->staff}` WHERE `id`=\"$ID\"");	
        ...
        foreach($qcheck->result() as $row):
            ...
            $access['staff_privileges']= unserialize($row->staff_privileges);
...
function get_mail_by_register($register_no){
         $qcheck="";
        $qcheck_register="";
       /* check the user is a student or not*/
       $qcheck_register=$this->db->query("SELECT * FROM `{$this->fixidb->register}` WHERE `registration_no`=\"$register_no\" LIMIT 1");     // for student
       if (!empty($qcheck_register) && $qcheck_register->num_rows() > 0)
       {
            $student_data_id = $qcheck_register->row()->student_data_id;
           // $ID=$qcheck->row()->id;
            ...
            {
                $row = $qcheck->row();    
                 return $qcheck->row()->student_email;
            }
