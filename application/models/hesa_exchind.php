...
class Hesa_exchind extends CI_Model {
    ...
    */    
    function update($args=array())
    {
      ...
      $this->db->update($this->fixidb->hesa_exchind,$args,array('id'=>$args['id']));
    ...
    */
    function add($args=array())
    {
     ...
     $this->db->insert($this->fixidb->hesa_exchind,$args);
    ...
    */
    function delete($user_id){
        ...
        if(isset($user_id)){
            $user_id        =   (int)$user_id;
            $this->db->delete($this->fixidb->hesa_exchind,array('id'    =>  $user_id));
    ...
    */
    function get_all(){
        ...
        $data =array();
        $this->db->db_select();
        $query=$this->db->get($this->fixidb->hesa_exchind);
        $i=0;
    ...
    function get_by_ID($ID=""){
        ...
        $data =array();
        $this->db->db_select();
        $query=$this->db->query("SELECT * FROM ".$this->fixidb->hesa_exchind." WHERE id='".$ID."' ORDER BY `id` ASC LIMIT 1");
        $i=0;
        foreach($query->list_fields() as $field):
         $fieldlist[$i] = $field;
         $i++;
