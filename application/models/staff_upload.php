...
class Staff_upload extends CI_Model {
    ...
    */    
    function update($args=array(),$ID="")
    {
      ...
      $this->db->update($this->fixidb->staff_upload,$args,array('id'=>$ID));
    ...
    */
    function add($args=array())
    {
	 ...
	 $this->db->insert($this->fixidb->staff_upload,$args);
    ...
    */
    function delete($ID){
        ...
        if(isset($ID)){
            $user_id=(int)$ID;
            $this->db->delete($this->fixidb->staff_upload,array('id'=>$user_id));
            return $user_id;
        }else{
          return FALSE;  
        }
    ...
    */
    function get_all(){
                     $this->db->db_select();
     $query      =   $this->db->get($this->fixidb->staff_upload);
     $i          =   0;
     $staff      =   array(); 
     $field_arr  =   array();
     ...
     function get_by_ID($ID="") {                  
        if(isset($this->upload_id) && $ID==""){ $ID = $this->upload_id; }
         $uploadfile  =   array();       
         $query =   $this->db->query("SELECT * FROM ".$this->fixidb->staff_upload." WHERE id='".$ID."' LIMIT 1");
         foreach($query->result() as $row) 
         {  
     ...
     function get_by_applicationID($ID) {
         $uploadfile  =   array();       
         $i        =   0;       
         $query =   $this->db->query("SELECT * FROM ".$this->fixidb->staff_upload." WHERE `student_data_id`='".$ID."' ORDER BY `id` ASC ");
         foreach($query->result() as $row) 
         {  
     ...
     function get_next_serial($ID) {
         $serial   = 1;        
         $query =   $this->db->query("SELECT * FROM ".$this->fixidb->staff_upload." WHERE `student_data_id`='".$ID."' ORDER BY `serial` DESC LIMIT 1");
